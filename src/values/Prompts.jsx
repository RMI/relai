...
export function createUserPrompt(projectName) {
  const userPrompt = `Analyze the following collection of internal documents and summarize relevant updates specifically related to the {PROJECT} project or any closely related digital transformation efforts. Your analysis should deeply examine the full content of the documents, not just their titles or metadata. Consider both direct mentions and indirect implications (e.g. related initiatives, dependencies, strategic directions).

Produce your output in the following structured JSON format:
{
   "relai_update": {
  "key_developments_and_decisions": "Summarize the most important updates, announcements, or decisions that affect the {PROJECT} project (e.g. changes in scope, approvals, strategy shifts, milestones reached).",
   "key_blockers_and_concerns": "Extract and explain any noted risks, open questions, resource constraints, delays, or other challenges that may hinder {PROJECT}'s progress.",
...
Emerging themes that might affect {PROJECT} (e.g. cloud adoption, rebranding priorities, evolving marketing strategies).
Be concise but specific in your output. Use clear language suitable for internal reporting.
`

  return userPrompt.replace(/{PROJECT}/gi, projectName);
}
